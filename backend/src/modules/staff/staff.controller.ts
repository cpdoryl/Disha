import { Controller, Get, Post, Patch, Param, Body, UseGuards, HttpCode, HttpStatus } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiBearerAuth, ApiParam } from '@nestjs/swagger';
import { StaffService } from 'src/services/staff.service';
import { JwtAuthGuard } from 'src/common/guards/jwt-auth.guard';
import { RolesGuard } from 'src/common/guards/roles.guard';
import { Roles } from 'src/common/decorators/roles.decorator';
import { EmploymentStatus } from 'src/database/entities';

@ApiTags('Staff')
@ApiBearerAuth()
@Controller('api/v2/staff')
@UseGuards(JwtAuthGuard, RolesGuard)
export class StaffController {
  constructor(private staffService: StaffService) {}

  @Post()
  @HttpCode(HttpStatus.CREATED)
  @Roles('ryl_admin', 'school_admin')
  @ApiOperation({ summary: 'Create staff record' })
  async createStaff(@Body() createStaffDto: any) {
    return this.staffService.createStaff(createStaffDto);
  }

  @Get(':id')
  @Roles('ryl_admin', 'school_admin', 'teacher')
  @ApiParam({ name: 'id', description: 'Staff ID' })
  @ApiOperation({ summary: 'Get staff details' })
  async getStaff(@Param('id') staffId: string) {
    return this.staffService.getStaff(staffId);
  }

  @Get('school/:schoolId')
  @Roles('ryl_admin', 'school_admin', 'teacher')
  @ApiParam({ name: 'schoolId', description: 'School ID' })
  @ApiOperation({ summary: 'Get staff by school' })
  async getStaffBySchool(@Param('schoolId') schoolId: string) {
    return this.staffService.getStaffBySchool(schoolId);
  }

  @Get('school/:schoolId/retention')
  @Roles('ryl_admin', 'school_admin')
  @ApiParam({ name: 'schoolId', description: 'School ID' })
  @ApiOperation({ summary: 'Get staff retention summary' })
  async getRetentionSummary(@Param('schoolId') schoolId: string) {
    return this.staffService.getRetentionSummary(schoolId);
  }

  @Patch(':id/status')
  @Roles('ryl_admin', 'school_admin')
  @ApiParam({ name: 'id', description: 'Staff ID' })
  @ApiOperation({ summary: 'Update staff employment status' })
  async updateEmploymentStatus(
    @Param('id') staffId: string,
    @Body() body: { status: EmploymentStatus; exitReasonCode?: string; exitReasonDetail?: string },
  ) {
    return this.staffService.updateEmploymentStatus(
      staffId,
      body.status,
      body.exitReasonCode,
      body.exitReasonDetail,
    );
  }
}
