...
export class AuditService {
  ...
  async logAction(auditDto: {
    ...
    auditLog.userId = auditDto.userId;
    auditLog.schoolId = auditDto.schoolId || '';
    auditLog.actionType = auditDto.actionType;
    auditLog.resourceType = auditDto.resourceType;
    auditLog.resourceId = auditDto.resourceId || '';
    auditLog.description = auditDto.description || '';
    auditLog.changesBefore = auditDto.changesBefore || {};
    auditLog.changesAfter = auditDto.changesAfter || {};
    auditLog.ipAddress = auditDto.ipAddress || '';
    auditLog.userAgent = auditDto.userAgent || '';
    auditLog.success = auditDto.success ?? true;
    auditLog.errorMessage = auditDto.errorMessage || '';
    auditLog.actionTimestamp = new Date();
  ...
  async getUserActivityReport(
    ...
    const filteredLogs = logs.filter(
      l =>
        l.actionTimestamp >= startDate && l.actionTimestamp <= endDate,
    );
    ...
    const timestamps = filteredLogs.map(l => l.actionTimestamp.getTime());
